...
app.iniciarEscutaCrm = function() {
    ...
    }, err => console.error("Erro ao escutar CRM:", err));
};
...
app.salvarClienteCRM = async function(e) {
    ...
    try {
        ...
        app.showToast("Erro ao salvar cliente: " + err.message, "error");
    }
...
app.apagarCliente = async function(id) {
    ...
    if(confirm("Apagar cliente? O histórico associado não será apagado, mas o perfil deixará de existir.")) { 
        try {
            ...
            app.showToast("Erro ao apagar: " + err.message, "error");
        }
    }
};
...
app.abrirChatCRM = function(clienteId, nomeCliente) {
    ...
    const elNome = document.getElementById('chatNomeCliente');
    if(elNome) elNome.innerHTML = `<span class="text-white-50">Atendimento Ativo:</span> <b class="text-accent fs-5">${nomeCliente}</b>`;
    
    const areaInput = document.getElementById('chatAreaInputGlobal');
    ...
    if(mD.length === 0) {
        ...
        mD.forEach(x => {
            if(x.sender === 'cliente' && !x.lidaAdmin) { 
                app.db.collection('mensagens').doc(x.id).update({lidaAdmin: true}).catch(e => console.error("Erro ao marcar como lida:", e));
            }
...
app.enviarMensagemChatGlobal = async function() {
    ...
    try {
        ...
        app.showToast("Erro ao enviar: " + err.message, "error");
    }
};
...
app.enviarAnexoChatGlobal = async function() {
    ...
    try {
        ...
        if(data.secure_url) {
            ...
            inp.value = ''; 
            app.showToast("Anexo enviado com sucesso!", "success");
        }
    } catch(e) { 
        console.error("Erro no upload:", e); 
        app.showToast("Falha no envio: " + e.message, "error"); 
    }
};
...
app.iniciarEscutaMensagensInternas = function() {
    ...
    }, err => console.error("Erro ao escutar chat interno:", err));
};
...
app.processarXML = function(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            app.showToast("Erro ao ler XML: " + err.message, "error");
        }
    };
...
app.salvarEntradaEstoque = async function(e) {
    ...
    try {
        if(id) {
            ...
            if(tr) {
                batch.update(app.db.collection('estoque').doc(id), { 
                    ...
                    sku: (tr.querySelector('.it-sku')?.value || '').trim(), 
                    qtd: parseFloat(tr.querySelector('.it-qtd')?.value || 0), 
                    custo: parseFloat(tr.querySelector('.it-custo')?.value || 0), 
                    venda: parseFloat(tr.querySelector('.it-venda')?.value || 0), 
                    ncm: tr.querySelector('.it-ncm')?.value || '', 
                    cfop: tr.querySelector('.it-cfop')?.value || '' 
                });
            ...
            document.querySelectorAll('#corpoItensNF tr').forEach(tr => {
                ...
                const sku = (tr.querySelector('.it-sku')?.value || '').trim(); 
                const q = parseFloat(tr.querySelector('.it-qtd')?.value || 0) || 0; 
                const c = parseFloat(tr.querySelector('.it-custo')?.value || 0) || 0; 
                const v = parseFloat(tr.querySelector('.it-venda')?.value || 0) || 0;
                if(desc !== '' && q > 0) {
                    totalCustoGlobalNF += (q * c);
                    batch.set(app.db.collection('estoque').doc(), { 
                        tenantId: app.t_id, fornecedor: fornecedor, nf: nf, sku: sku, desc: desc, qtd: q, custo: c, venda: v, 
                        ncm: tr.querySelector('.it-ncm')?.value || '', cfop: tr.querySelector('.it-cfop')?.value || '', 
                        usuarioEntrada: app.user_nome, dataEntrada: new Date().toISOString() 
                    });
                }
            });
            if(totalCustoGlobalNF === 0) {
                app.showToast("Nenhum item válido para dar entrada.", "error");
                return;
            }
        ...
        await batch.commit(); 
        app.showToast("Estoque Atualizado e Compra Lançada!", "success"); 
        e.target.reset(); 
        const modalInst = bootstrap.Modal.getInstance(document.getElementById('modalNF'));
        if(modalInst) modalInst.hide();
    } catch(err) {
        console.error("Erro ao salvar entrada estoque:", err);
        app.showToast("Erro: " + err.message, "error");
    }
};
...
app.apagarProduto = async function(id) {
    ...
    if(confirm("Excluir produto da base?")) { 
        try {
            await app.db.collection('estoque').doc(id).delete(); 
            app.registrarAuditoriaGlobal("Estoque", `Deletou produto (ID: ${id}).`); 
...
app.iniciarEscutaOS = function() {
    ...
    }, err => console.error("Erro ao escutar OS:", err));
};
...
app.confirmarAtribuicaoBox = async function() {
    ...
    try {
        ...
        app.showToast("Encaminhado para execução!", "success"); 
        const modalInst = bootstrap.Modal.getInstance(document.getElementById('modalAtribuicaoBox'));
        if(modalInst) modalInst.hide();
    } catch(err) {
        console.error("Erro ao confirmar atribuição:", err);
        app.showToast("Erro: " + err.message, "error");
    }
};

app.abrirModalNotificacaoWhatsApp = function(id, tipo) {
    const elId = document.getElementById('whats_os_id');
    const elTipo = document.getElementById('whats_tipo_msg');
    ...
    if(os) {
        if(tipo === 'pronto') { 
            ...
            if(texto) texto.innerText = `Deseja notificar o cliente ${os.cliente} via WhatsApp?`; 
        }
...
app.dispararWhatsAppAtivo = function() {
    ...
    if (os && os.celular) {
        let texto = ''; 
        if (tipo === 'pronto') texto = `Olá ${os.cliente}, ótimas notícias da *${app.t_nome}*! 🚀\nO serviço no seu *${os.veiculo}* foi concluído. Por favor, confirme o melhor horário para retirada.`;
        window.open(`https://wa.me/55${os.celular.replace(/\D/g, '')}?text=${encodeURIComponent(texto)}`, '_blank');
        const h = os.historico || []; 
        h.push({ data: new Date().toISOString(), usuario: app.user_nome, acao: `Aviso CONCLUSÃO via WhatsApp.` });
        app.db.collection('ordens_servico').doc(id).update({ historico: h }).catch(e => console.error("Erro ao atualizar histórico:", e));
...
app.verificarStatusLink = function() {
    ...
    if(!a) return;
    const statusEl = document.getElementById('os_status');
    const idEl = document.getElementById('os_id');
    if (statusEl?.value === 'aprovacao' && idEl?.value) {
...
app.adicionarLinhaPeca = function(desc, ncm, qtd, custo, venda, idEstoque, isMaoObra) {
    ...
    const corpo = document.getElementById('listaPecasCorpo');
    if(corpo) corpo.appendChild(tr); 
    app.calcularTotalOS();
};
...
app.calcularTotalOS = function() {
    ...
    document.querySelectorAll('#listaPecasCorpo tr').forEach(tr => {
        ...
        const q = parseFloat(tr.querySelector('.peca-qtd')?.value || 0) || 0, 
              v = parseFloat(tr.querySelector('.peca-venda')?.value || 0) || 0, 
              c = parseFloat(tr.querySelector('.peca-custo')?.value || 0) || 0;
...
app.salvarOS = async function() {
    try {
        ...
        const cpfOS = document.getElementById('os_cliente_cpf')?.value || '', 
              clienteOS = (document.getElementById('os_cliente')?.value || '').trim(), 
              telOS = (document.getElementById('os_celular')?.value || '').trim();

        let cId = '';
        if(clienteOS && !app.bancoCrm.find(c => c.nome.toLowerCase() === clienteOS.toLowerCase())) {
            const d = await app.db.collection('clientes_base').add({ tenantId: app.t_id, nome: clienteOS, telefone: telOS, documento: cpfOS, anotacoes: "Criado via O.S." }); 
            cId = d.id;
        } else { 
        ...
        const novoStatus = document.getElementById('os_status')?.value || 'patio';
        ...
        const payload = { 
            ...
            chk_combustivel: document.getElementById('chk_combustivel')?.checked || false, 
            chk_arranhado: document.getElementById('chk_arranhado')?.checked || false, 
            chk_bateria: document.getElementById('chk_bateria')?.checked || false, 
            chk_pneus: document.getElementById('chk_pneus')?.checked || false, 
            pecas: pecasArray, 
...
app.abrirFaturamentoDireto = function(id) {
    ...
    if(app.osParaFaturar) {
        const el = document.getElementById('fat_valor_total');
        if(el) el.innerText = `R$ ${(app.osParaFaturar.total||0).toFixed(2).replace('.',',')}`;
    }
...
app.processarFaturamentoCompleto = async function() {
    ...
    const fp = document.getElementById('fat_metodo')?.value || '', 
          parcelasText = document.getElementById('fat_parcelas')?.value || '';
    ...
    try {
        ...
        h.push({ data: new Date().toISOString(), usuario: app.user_nome, acao: `FATURAMENTO CONCLUÍDO. Comissões lançadas no Financeiro.` });
        ...
        await batch.commit(); 
        app.registrarAuditoriaGlobal("Faturamento", `Faturou placa ${app.osParaFaturar.placa}.`);
        ...
        app.showToast("Erro no faturamento: " + err.message, "error");
    }
};
...
app.abrirModalFinanceiro = function(mode='nova', tipo='', id='') {
    ...
    if(form) form.reset(); 
    const idField = document.getElementById('fin_id');
    if(idField) idField.value = '';
    app.popularSelectVinculoRH(); 
    const divVinculo = document.getElementById('div_vinculo_rh');
    if(divVinculo) {
        divVinculo.style.display = 'none'; 
        const vincEl = document.getElementById('fin_vinculo_rh');
        if(vincEl) vincEl.removeAttribute('required');
    }
    ...
    const tipoField = document.getElementById('fin_tipo');
    if(tipoField) tipoField.value = tipo;
    const titulo = document.getElementById('fin_titulo');
    if(titulo) {
        titulo.innerHTML = tipo === 'receita' ? '<i class="bi bi-plus-circle text-success me-2"></i> Receita Avulsa' : '<i class="bi bi-dash-circle text-danger me-2"></i> Lançar Despesa / NF';
    }
    const dataField = document.getElementById('fin_data');
    ...
    if(mode === 'edit') {
        ...
        if(f) {
            ...
            if(document.getElementById('fin_categoria')) { 
                document.getElementById('fin_categoria').value = f.categoria || 'OUTROS'; 
                document.getElementById('fin_categoria').dispatchEvent(new Event('change')); 
            }
            if(document.getElementById('fin_vinculo_rh') && f.pessoaId) { 
                document.getElementById('fin_vinculo_rh').value = `${f.pessoaId}|${f.pessoaNome}`; 
            }
            ...
            if(divParc) divParc.style.display = 'none';
        }
    } else { 
        const divStatus = document.getElementById('divStatusEdit');
        if(divStatus) divStatus.style.display = 'none'; 
        const divParc = document.getElementById('divParcelas');
        if(divParc) divParc.style.display = 'block'; 
        if(tipo === 'receita') {
            const parcEl = document.getElementById('fin_parcelas');
...
app.verificarPgtoFinManual = function() {
    ...
    if(d) { 
        if(f.includes('Parcelado') || f.includes('Boleto')) {
            ...
            d.style.display = 'none'; 
            const parc = document.getElementById('fin_parcelas');
...
app.salvarLancamentoFinanceiro = async function(e) {
    ...
    try {
        ...
        const valorTotal = parseFloat(document.getElementById('fin_valor')?.value || 0) || 0; 
        const dataInicial = new Date(document.getElementById('fin_data')?.value || new Date()); 
        ...
        if(id) {
            const m = prompt("ATENÇÃO: Modificando DRE. Digite a JUSTIFICATIVA:"); 
            if(!m || m.trim() === '') return app.showToast("Justificativa obrigatória.", "error");
            const sts = document.getElementById('fin_status')?.value || 'pendente';
            ...
            app.showToast(`Documento Atualizado.`, "success"); 
            app.registrarAuditoriaGlobal("Financeiro", `Editou título. Justificativa: ${m}`);
        } else {
            const batch = app.db.batch(); 
            let nP = 1; 
            if(fp.includes('Boleto') || fp.includes('Cartão') || fp.includes('Parcelado')) {
                nP = parseInt(document.getElementById('fin_parcelas')?.value || 1) || 1;
            }
...
app.iniciarEscutaFinanceiro = function() { 
    ...
    }, err => console.error("Erro ao escutar financeiro:", err)); 
};

app.filtrarFinanceiro = function(silent = false) { 
    const inicioEl = document.getElementById('filtroFinInicio');
    const fimEl = document.getElementById('filtroFinFim');
    app.filtroFinDataInicio = inicioEl?.value || null; 
    app.filtroFinDataFim = fimEl?.value || null;
...
app.renderizarFinanceiroGeral = function() {
    ...
    if(pagarCorpo) pagarCorpo.innerHTML = hPagar || '<tr><td colspan="7" class="text-center text-white-50 py-5">Nenhuma dívida pendente.</td></tr>';
...
app.exportarRelatorioFinanceiro = function() {
    try {
        ...
        y += 10;
        doc.setFontSize(12); 
        doc.text(`RELATÓRIO GERENCIAL - DRE`, 105, y, { align: "center" }); 
        y += 20;
        let bR = []; 
        app.bancoFinFiltrado.filter(x=>x.tipo==='ENTRADA').forEach(x => { 
            bR.push([x.vencimento?.split('-').reverse().join('/') || '', x.desc||'', x.metodo||'', `R$ ${x.valor.toFixed(2)}`, x.status?.toUpperCase() || '']); 
        });
        if(bR.length > 0) { 
            doc.autoTable({ startY: y, head: [['Data Base', 'Cliente/Referência', 'Modalidade', 'Vlr.', 'Status']], body: bR }); 
            y = doc.lastAutoTable.finalY + 15; 
        }
        let bP = []; 
        app.bancoFinFiltrado.filter(x=>x.tipo==='SAIDA').forEach(x => { 
            bP.push([x.vencimento?.split('-').reverse().join('/') || '', x.desc||'', x.metodo||'', `R$ ${x.valor.toFixed(2)}`, x.status?.toUpperCase() || '']); 
        });
        ...
        app.showToast("Erro no PDF: " + e.message, "error"); 
    }
};
...
app.abrirModalValeRH = function(idFunc, nomeFunc) { 
    if(document.getElementById('vale_id_func')) document.getElementById('vale_id_func').value = idFunc; 
    if(document.getElementById('vale_nome_func')) document.getElementById('vale_nome_func').value = nomeFunc; 
    if(document.getElementById('lblNomeValeFunc')) document.getElementById('lblNomeValeFunc').innerText = nomeFunc; 
...
app.confirmarValeRH = async function(e) {
    ...
    try {
        ...
        const valor = parseFloat(document.getElementById('vale_valor')?.value || 0) || 0; 
        ...
        console.error("Erro ao confirmar vale:", err);
        app.showToast("Erro: " + err.message, "error");
    }
};
...
app.iniciarEscutaLixeira = function() {
    ...
    }, err => console.error("Erro ao escutar lixeira:", err));
};

app.apagarOS = async function() {
    if(app.t_role !== 'admin') return app.showToast("Bloqueado. Exige perfil Dono.", "error");
    const m = prompt("JUSTIFICATIVA REAL:"); 
    if(!m || m.trim() === '') return app.showToast("Abortado.", "error");
    const id = document.getElementById('os_id')?.value || ''; 
    const osC = app.bancoOSCompleto.find(x => x.id === id);
    if(osC) { 
        try {
            await app.registrarAuditoriaGlobal(`O.S Cancelada: ${osC.placa}`, `Motivo: ${m}`); 
            await app.db.collection('ordens_servico').doc(id).delete(); 
            app.showToast("Movido p/ Lixeira.", "success"); 
        } catch(err) {
            console.error("Erro ao apagar OS:", err);
            app.showToast("Erro: " + err.message, "error");
        }
    }
    const modalInst = bootstrap.Modal.getInstance(document.getElementById('modalOS'));
    ...
    try {
        var w = cloudinary.createUploadWidget({ 
            ...
            if (!err && res && res.event === "success") { 
                ...
                app.historicoOSAtual.push({ data: new Date().toISOString(), usuario: app.user_nome, acao: "Foto enviada." }); 
        ...
        console.error("Erro ao configurar Cloudinary:", e);
    }
};
...
app.removerFoto = function(index) { 
    ...
    app.historicoOSAtual.push({ data: new Date().toISOString(), usuario: app.user_nome, acao: "Excluiu Foto." }); 
    app.renderizarGaleria(); 
};
...
app.exportarPDFMenechelli = async function() {
    ...
    try {
        ...
        doc.text(`LAUDO DE INTEGRIDADE, SERVIÇO E ORÇAMENTO`, w/2, 30, { align: "center" }); 
        y = 50; 
        doc.setTextColor(0, 0, 0);
        doc.setDrawColor(200, 200, 200); 
        doc.rect(15, y, w-30, 25); 
        doc.setFont("helvetica", "bold"); 
        doc.text(`Proprietário:`, 20, y+8); 
        doc.setFont("helvetica", "normal"); 
        doc.text(document.getElementById('os_cliente')?.value || '', 50, y+8); 
        doc.setFont("helvetica", "bold"); 
        doc.text(`Contato:`, 130, y+8); 
        doc.setFont("helvetica", "normal"); 
        doc.text(document.getElementById('os_celular')?.value || '', 150, y+8); 
        doc.setFont("helvetica", "bold"); 
        ...
        doc.setFont("helvetica", "normal"); 
        doc.text(document.getElementById('os_veiculo')?.value || '', 148, y+18); 
        y += 35;
        doc.setFont("helvetica", "bold"); 
        doc.setFontSize(12); 
        doc.text(`QUEIXA NA RECEPÇÃO`, 15, y); 
        doc.line(15, y+2, w-15, y+2); 
        y += 10; 
        doc.setFont("helvetica", "normal"); 
        doc.setFontSize(10); 
        const txtQ = doc.splitTextToSize(document.getElementById('os_relato_cliente')?.value || '', w - 30); 
        doc.text(txtQ, 15, y); 
        y += (txtQ.length * 6) + 10;
        doc.setFont("helvetica", "bold"); 
        doc.setFontSize(12); 
        ...
        doc.setFont("helvetica", "normal"); 
        doc.setFontSize(10); 
        const txtL = doc.splitTextToSize(document.getElementById('os_diagnostico')?.value || '', w - 30); 
        doc.text(txtL, 15, y); 
        y += (txtL.length * 6) + 10;
        let tB = []; 
        document.querySelectorAll('#listaPecasCorpo tr').forEach(tr => { 
            tB.push([
                tr.querySelector('.peca-desc')?.value || '', 
                tr.querySelector('.peca-qtd')?.value || '', 
                `R$ ${tr.querySelector('.peca-venda')?.value || 0}`, 
                `R$ ${tr.querySelector('.peca-total')?.value || 0}`
            ]); 
        });
        ...
        if (app.fotosOSAtual.length > 0) { 
            ...
            for (let i = 0; i < app.fotosOSAtual.length; i++) { 
                ...
                if(!b64) continue;
                if (iX + 45 > w - 15) { iX = 15; iY += 53; } 
                if (iY + 45 > 280) { doc.addPage(); iY = 20; iX = 15; } 
...
// =====================================================================
// 12. CÉREBRO DA I.A. GEMINI 1.5 FLASH (CORRIGIDO E CONSOLIDADO)
// =====================================================================
app.minhaGeminiKey = null;
app.iaTrabalhando = false;

// Função para carregar chave da API do Firestore
app.carregarGeminiKey = async function() {
    ...
    try {
        ...
        if (doc.exists) {
            const data = doc.data();
            // Suporte a múltiplos nomes de campo para compatibilidade
            app.minhaGeminiKey = data.geminiKey || data.gemini || data.apiGemini || data.api_gemini || data.apiKeyGemini || null;
            if (app.minhaGeminiKey) {
                console.log("[IA] Chave Gemini carregada com sucesso para tenant:", app.t_id);
            } else {
                console.warn("[IA] Chave Gemini não encontrada no documento da oficina");
            }
            return app.minhaGeminiKey;
        } else {
            console.warn("[IA] Documento da oficina não encontrado");
        }
    } catch (e) { 
        console.error("[IA] Erro ao buscar chave Gemini:", e); 
    }
    return null;
};
...
app.iniciarEscutaIA = function() {
    if(app.t_id) {
        // Listener em tempo real para atualizar a chave se mudar no Firestore
        app.db.collection('oficinas').doc(app.t_id).onSnapshot(doc => { 
            if(doc.exists) {
                const data = doc.data();
                app.minhaGeminiKey = data.geminiKey || data.gemini || data.apiGemini || data.api_gemini || data.apiKeyGemini || null;
                if(app.minhaGeminiKey) {
                    console.log("[IA] Chave atualizada via listener");
                }
            }
        }, err => console.error("[IA] Erro no listener da oficina:", err));
    }
    // Listener para base de conhecimento RAG
    ...
    }, err => console.error("[IA] Erro ao escutar conhecimento:", err));
};
...
app.salvarConhecimentoIA = async function(textoAvulso = null) {
    const inputEl = document.getElementById('iaConhecimentoTexto');
    const val = textoAvulso || (inputEl?.value.trim() || '');
    if(!val) return app.showToast("Conteúdo vazio.", "warning");
    try {
        ...
        if(!textoAvulso && inputEl) inputEl.value = '';
    } catch(err) {
        console.error("[IA] Erro ao salvar conhecimento:", err);
        app.showToast("Erro: " + err.message, "error");
    }
};

app.apagarConhecimentoIA = async function(id) { 
    if(confirm("Apagar memória?")) { 
        try {
            await app.db.collection('conhecimento_ia').doc(id).delete(); 
            app.showToast("Apagada.", "success"); 
        } catch(err) {
            console.error("[IA] Erro ao apagar conhecimento:", err);
            app.showToast("Erro: " + err.message, "error");
        }
    } 
};
...
app.processarArquivoParaIA = function(event) { 
    ...
    r.onload = async function(e) { 
        try {
            ...
            console.error("[IA] Erro ao processar arquivo:", err);
            app.showToast("Erro: " + err.message, "error");
        }
    }; 
    r.readAsText(file); 
...
// MOTOR GEMINI 1.5 FLASH - VERSÃO CORRIGIDA SEM THROW
app.chamarGemini = async function(promptCompleto) {
    // Garante que a chave está carregada
    if (!app.minhaGeminiKey) {
        console.log("[IA] Chave ausente na memória. Buscando no Firestore...");
        await app.carregarGeminiKey();
        if (!app.minhaGeminiKey) { 
            console.error("[IA] Abortado: Chave da API não encontrada"); 
            app.showToast("Erro na IA: Chave da API não configurada", "error");
            return "Erro: Chave da API não configurada. Contate o administrador."; 
        }
    }
    ...
    try {
        ...
        if (!res.ok || data.error) {
            const msg = data.error ? data.error.message : `Erro HTTP ${res.status}`;
            console.error("[IA] Erro na API Gemini:", msg);
            app.showToast("Erro na IA: " + msg, "error");
            return "Erro: " + msg;
        }
        
        const respostaTexto = data.candidates?.[0]?.content?.parts?.[0]?.text;
        if (!respostaTexto) {
            console.error("[IA] Resposta vazia ou formato irreconhecível");
            app.showToast("Erro na IA: Resposta inválida", "error");
            return "Erro: Formato de resposta irreconhecível.";
        }
        
        return respostaTexto;
        
    } catch(e) { 
        console.error("[IA] Falha crítica ao chamar Gemini:", e); 
        app.showToast("Erro na IA: " + e.message, "error");
