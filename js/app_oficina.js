...
app.dispararWhatsAppAtivo = function() {
    ...
    if (os && os.celular) {
        ...
        const h = os.historico || []; h.push({ data: new Date().toISOString(), usuario: app.user_nome, acao: `WhatsApp CONCLUSÃO.` });
...
app.salvarOS = async function() {
    try {
        ...
        app.historicoOSAtual.push({ data: new Date().toISOString(), usuario: app.user_nome, acao: id ? `Editou O.S. Status: ${novoStatus.toUpperCase()}` : "Abriu Ficha." });
...
app.confirmarValeRH = async function(e) {
    ...
    batch.set(app.db.collection('financeiro').doc(), { tenantId: app.t_id, tipo: 'SAIDA', categoria: 'VALE', desc: `[VALE] ${nomeFunc} - ${motivo}`, valor: valor, parcelaAtual: 1, totalParcelas: 1, metodo: 'Dinheiro', vencimento: dataH.split('T')[0], status: 'pago', pessoaId: idFunc, pessoaNome: nomeFunc, origem: 'MANUAL', referenciaId: null, data: dataH });
...
app.configurarCloudinary = function() {
    ...
    try {
        var w = cloudinary.createUploadWidget({ cloudName: app.CLOUDINARY_CLOUD_NAME, uploadPreset: app.CLOUDINARY_UPLOAD_PRESET, sources: ['local', 'camera'], language: 'pt' }, (err, res) => {
            if (!err && res && res.event === "success") { app.fotosOSAtual.push(res.info.secure_url); app.historicoOSAtual.push({ data: new Date().toISOString(), usuario: app.user_nome, acao: "Foto." }); app.renderizarGaleria(); }
        });
