...
app.dispararWhatsAppAtivo = function() {
    ...
    bootstrap.Modal.getInstance(document.getElementById('modalNotificaWhatsApp')).show(); // Corrigido para não travar
...
// =====================================================================
// 13. CÉREBRO DA I.A. (API KEY NO FIREBASE + AUTO-FALLBACK SEGURO)
// =====================================================================
app.iaTrabalhando = false; 
app.minhaGeminiKey = null;

app.salvarKeyOficina = async function() {
    const key = document.getElementById('oficinaGeminiKey').value.trim();
    if(!key) { app.showToast("Cole uma Chave API válida.", "warning"); return; }
    
    try {
        // O PULO DO GATO: Salva a chave direto no banco de dados da oficina (não no LocalStorage)
        // Isso garante que todos os dispositivos (celular do mecânico, PC do gerente) puxem a mesma chave
        await app.db.collection('oficinas').doc(app.t_id).set({
            apiKeys: { gemini: key }
        }, { merge: true });
        
        app.showToast("Chave da IA salva com sucesso no Banco de Dados!", "success");
        setTimeout(() => { document.getElementById('oficinaGeminiKey').value = '**************************'; }, 1000);
    } catch (err) {
        console.error(err);
        app.showToast("Erro ao salvar a chave no banco.", "error");
    }
};

app.iniciarEscutaIA = function() {
    // Escuta a chave no Firebase em tempo real
    if(app.t_id) {
        app.db.collection('oficinas').doc(app.t_id).onSnapshot(doc => {
            if(doc.exists) {
                const data = doc.data();
                if(data.apiKeys && data.apiKeys.gemini) {
                    app.minhaGeminiKey = data.apiKeys.gemini;
                    const inputKey = document.getElementById('oficinaGeminiKey');
                    if(inputKey && !inputKey.value.includes('*')) inputKey.value = '**************************';
                }
            }
        });
    }
...
app.chamarGemini = async function(prompt, sysInstruction) {
    // Agora ele puxa a chave que o banco de dados enviou na escuta em tempo real
    const key = app.minhaGeminiKey;
    
    if(!key || key.includes('*')) { 
        app.showToast("O Gerente precisa ir no menu Treinamento IA (RAG) e ativar a chave da API.", "error"); 
        return "Erro: Google Gemini API Key ausente no sistema desta oficina."; 
    }
    ...
    try {
        ...
        if (!res.ok) {
            console.warn("Rota 2.5-flash bloqueada. Acionando fallback automático...");
        ...
        console.error(e);
        return "A I.A. bloqueou a requisição. Verifique se o Gerente colou a chave corretamente ou se a cota acabou. Erro: " + e.message; 
    }
};
