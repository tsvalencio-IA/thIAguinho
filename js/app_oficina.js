...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Controlo de Acesso Dinâmico (RBAC)
...
app.construirMenuLateral = function() {
    ...
    if (app.t_role === 'admin') {
        ...
        html += `<a class="nav-link" onclick="app.mostrarTela('tela_equipe', 'Gestão da Equipa', this)"><i class="bi bi-people-fill text-success"></i> Equipa e Acessos</a>`;
    }
    menu.innerHTML = html;
};
...
app.salvarClienteCRM = async function(e) {
    ...
    if(id) { await app.db.collection('clientes_base').doc(id).update(payload); app.showToast("Ficha de cliente atualizada."); } 
    else { await app.db.collection('clientes_base').add(payload); app.showToast("Novo cliente registado."); }
...
app.enviarWhatsAppAprovacao = function() {
    ...
    if(!cel) return app.showToast("Telemóvel não informado.", "error");
    
    // O LINK REAL DO PORTAL DO CLIENTE (projeto_oficina.html)
    ...
    let txt = `Olá ${nome}! A O.S. do seu veículo foi atualizada na *${app.t_nome}*.\nAceda ao nosso portal oficial para visualizar o orçamento detalhado, ver fotos e contactar-nos pelo chat da plataforma:\n👉 ${u}`;
    if(cZ && cZ.usuario) { txt += `\n\n*As Suas Credenciais Seguras:*\nLogin: ${cZ.usuario}\nPIN: ${cZ.senha}`; }
...
app.abrirChatCRM = function(clienteId, nomeCliente) {
    ...
    if(mD.length === 0) {
        ...
        mD.forEach(x => {
            ...
            // Tratamento das mídias enviadas
            if(x.fileUrl) {
...
app.enviarAnexoChatGlobal = async function() {
    ...
    app.showToast("A efetuar upload e enviar o anexo...", "warning");
    try {
        ...
        console.error(e);
        app.showToast("Falha ao enviar ficheiro via Cloudinary.", "error");
    }
};


// =====================================================================
// 5. ESTOQUE E ENTRADA DE N.F. (XML VERDADEIRO E INFINITO)
// =====================================================================
app.iniciarEscutaEstoque = function() {
    app.db.collection('estoque').where('tenantId', '==', app.t_id).onSnapshot(snap => {
        ...
        if(tbody) {
            tbody.innerHTML = app.bancoEstoque.map(p => `<tr><td><small class="text-white-50">${p.fornecedor||'N/A'}</small><br><span class="badge bg-primary">NF: ${p.nf||'S/N'}</span></td><td><strong class="text-white">${p.desc}</strong></td><td><span class="badge bg-secondary px-3 py-2 fs-6 shadow-sm">${p.qtd} un</span></td><td class="gestao-only text-danger fw-bold">R$ ${p.custo.toFixed(2)}</td><td class="text-success fw-bold fs-6">R$ ${p.venda.toFixed(2)}</td><td class="admin-only text-end"><button class="btn btn-sm btn-outline-info me-1 shadow-sm" onclick="app.abrirModalNF('edit', '${p.id}')"><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-outline-danger shadow-sm" onclick="app.apagarProduto('${p.id}')"><i class="bi bi-trash-fill"></i></button></td></tr>`).join('');
        }
...
app.processarXML = function(event) {
    ...
    reader.onload = function(e) {
        ...
        document.getElementById('corpoItensNF').innerHTML = ''; // Limpa as linhas manuais
        for(let i=0; i<det.length; i++) {
            ...
            if(prod) {
                ...
                app.adicionarLinhaNF(desc, ncm, cfop, qtd, vUnCom, (vUnCom * 1.8)); // Sugere margem de 80%.
            }
        }
...
app.salvarEntradaEstoque = async function(e) {
    ...
    await batch.commit();
    app.showToast("Ação processada com sucesso!", "success");
    e.target.reset(); 
...
app.apagarProduto = async function(id) {
    ...
    if(confirm("Excluir produto do stock?")) { await app.db.collection('estoque').doc(id).delete(); }
};
...
// =====================================================================
// 8. FATURAMENTO E DRE (REGRAS EVOLUTION INTEGRAIS)
// =====================================================================
...
// =====================================================================
// 9. FINANCEIRO AVULSO (EDIÇÃO E INSERÇÃO)
// =====================================================================
...
app.renderizarFinanceiroGeral = function() {
    ...
    app.bancoFin.sort((a,b) => new Date(a.vencimento) - new Date(b.vencimento)).forEach(f => {
        ...
        const html = `<tr><td class="text-white-50"><i class="bi bi-calendar-event me-2"></i> ${new Date(f.vencimento).toLocaleDateString('pt-BR')}</td><td class="text-white fw-bold">${f.desc}</td><td><span class="badge bg-dark border border-secondary px-3 py-1 text-white-50">${f.parcelaAtual}/${f.totalParcelas}</span></td><td class="text-info small fw-bold">${f.metodo || 'Dinheiro'}</td><td class="${cor} fw-bold fs-6">R$ ${f.valor.toFixed(2).replace('.',',')}</td><td>${st}</td><td class="gestao-only">${btn} <button class="btn btn-sm btn-outline-info me-1" onclick="app.abrirModalFinanceiro('edit','${f.tipo}','${f.id}')"><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-link text-danger admin-only" onclick="app.db.collection('financeiro').doc('${f.id}').delete()"><i class="bi bi-trash"></i></button></td></tr>`;
...
app.iniciarEscutaEquipe = function() {
    app.db.collection('funcionarios').where('tenantId', '==', app.t_id).onSnapshot(snap => {
        ...
        if(!tbody) return;
        if(snap.empty) { tbody.innerHTML = '<tr><td colspan="5" class="text-center text-white-50 py-5 fs-5">Sem equipa.</td></tr>'; return; }
...
app.apagarFuncionario = async function(id) { if(confirm("Deseja bloquear permanentemente o acesso deste utilizador?")) { await app.db.collection('funcionarios').doc(id).delete(); app.showToast("Acesso destruído.", "success"); } };
...
// =====================================================================
// 11. CLOUDINARY E LAUDO PDF (COM MINIATURAS)
// =====================================================================
...
app.exportarPDFMenechelli = async function() {
    const btn = document.getElementById('btnGerarPDF'); btn.innerHTML = 'Processando...'; btn.disabled = true; const placa = document.getElementById('os_placa').value;
    try {
...
// =====================================================================
// 12. J.A.R.V.I.S E IA (GEMINI 2.5 FLASH)
// =====================================================================
...
app.apagarConhecimentoIA = async function(id) { if(confirm("Deseja apagar este conhecimento da I.A.?")) { await app.db.collection('conhecimento_ia').doc(id).delete(); app.showToast("Ficheiro removido.", "success"); } };

app.processarArquivoParaIA = function(event) {
    const file = event.target.files[0]; if(!file) return;
    const statusLabel = document.getElementById('iaFileStatus'); if(statusLabel) { statusLabel.className = "text-warning fw-bold d-block text-center"; statusLabel.innerText = "A ler ficheiro e injetar conhecimento..."; }
    const reader = new FileReader();
...
app.perguntarJarvis = async function() {
    ...
    if(respDiv) { respDiv.classList.remove('d-none'); respDiv.innerHTML = '<span class="spinner-border text-info spinner-border-sm me-2"></span> J.A.R.V.I.S a analisar dados...'; }
...
app.jarvisAnalisarRevisoes = async function() {
    ...
    div.innerHTML = '<span class="spinner-border text-warning spinner-border-sm me-2"></span> A escrutinar o Histórico Morto...';
    const historicoMorto = app.bancoOSCompleto.filter(o => o.status === 'entregue');
    if(historicoMorto.length === 0) { div.innerHTML = '<span class="text-white-50">Não há registos suficientes para remarketing.</span>'; return; }
