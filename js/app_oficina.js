...
// =====================================================================
// 1. CONFIGURAÇÃO E SESSÃO DA OFICINA
// =====================================================================
app.firebaseConfig = {
...
app.abrirModalCRM = function(mode = 'nova', id = '') {
    ...
    if (mode === 'edit') {
        const c = app.bancoCrm.find(x => x.id === id);        if (c) {
...
app.enviarAnexoChatGlobal = async function() {
    ...
    try {
        ...
        if (data.secure_url) {
            await app.db.collection('mensagens').add({
                tenantId: app.t_id, clienteId: app.chatActiveClienteId, sender: 'admin',                text: "📎 Mídia:", fileUrl: data.secure_url, fileType: data.resource_type,
                lidaCliente: false, timestamp: firebase.firestore.FieldValue.serverTimestamp()
            });
            inp.value = ''; app.showToast("Enviado!", "success");
        }
    } catch (e) { app.showToast("Falha no upload.", "error"); }
};
    ...
    reader.onload = function(e) {
        ...
        for (let i = 0; i < det.length; i++) {
            ...
            if (prod) {
                ...
                const cfop = prod.getElementsByTagName("CFOP")[0]?.textContent || '';                const qtd = parseFloat(prod.getElementsByTagName("qCom")[0]?.textContent || 0);
...
// =====================================================================
// 6. KANBAN E PRONTUÁRIO (CORREÇÃO TOTAL)
// =====================================================================
...
app.mudarStatusRapido = async function(id, novoStatus) {
    ...
    h.push({ data: new Date().toISOString(), usuario: app.user_nome, acao: `Card movido para: ${novoStatus.toUpperCase()}` });
...
app.adicionarDoEstoque = function() {
    ...
    const opt = sel.options[sel.selectedIndex];    app.adicionarLinhaPeca(opt.dataset.desc, 1, parseFloat(opt.dataset.custo), parseFloat(opt.dataset.venda), sel.value, false);
    sel.value = '';
};
...
app.processarFaturamentoCompleto = async function() {
    ...
    if (app.osParaFaturar.pecas && !app.osParaFaturar.baixaEstoqueFeita) {
        for (const p of app.osParaFaturar.pecas) {            if (p.idEstoque) {
...
app.salvarLancamentoFinanceiro = async function(e) {
    e.preventDefault();
    await app.db.collection('financeiro').add({
        tenantId: app.t_id, tipo: document.getElementById('fin_tipo').value,
        desc: document.getElementById('fin_desc').value, valor: parseFloat(document.getElementById('fin_valor').value),
        vencimento: new Date(document.getElementById('fin_data').value).toISOString(),
        status: 'pago', metodo: 'Dinheiro'
    });
...
// =====================================================================
// 8. GESTÃO DE EQUIPE (COM FINANCEIRO)
// =====================================================================
app.iniciarEscutaEquipe = function() {
    app.db.collection('funcionarios').where('tenantId', '==', app.t_id).onSnapshot(snap => {
        ...
        tbody.innerHTML = snap.docs.map(doc => {
            ...
            const comissoes = app.bancoOSCompleto
            ...
            return `<tr>
                ...
                <td>${nAcesso}</td>                <td class="text-warning">${f.comissao || 0}%</td>
                <td>
                    <div class="small text-white-50">Comissões: R$ ${comissoes.toFixed(2)}</div>
                    <div class="small text-danger">Adiantamentos: -R$ ${avancos.toFixed(2)}</div>
                    <div class="small text-success fw-bold">A Receber: R$ ${saldo.toFixed(2)}</div>
                </td>
                <td class="admin-only"><button class="btn btn-sm btn-outline-warning shadow-sm px-3" onclick="app.gerenciarFinancasFunc('${doc.id}', '${f.nome}')"><i class="bi bi-wallet2 me-1"></i> Financ.</button> <button class="btn btn-sm btn-outline-danger shadow-sm px-3" onclick="app.apagarFuncionario('${doc.id}')"><i class="bi bi-slash-circle me-1"></i> Revogar</button></td>
            </tr>`;
        }).join('');
    });
};
...
app.salvarFuncionario = async function(e) {
    ...
    app.showToast("Acesso criado.", "success"); e.target.reset(); bootstrap.Modal.getInstance(document.getElementById('modalEquipe')).hide();
};

app.apagarFuncionario = async function(id) { if (confirm("Deseja bloquear o acesso?")) { await app.db.collection('funcionarios').doc(id).delete(); app.showToast("Acesso destruído.", "success"); } };

app.gerenciarFinancasFunc = async function(id, nome) {
    const val = prompt(`Registrar Adiantamento/Desconto para ${nome}. Use valor negativo para estorno:`);
    if (val !== null && val !== '') {
        ...
        if (!isNaN(v)) {
            ...
            avancos.push({ valor: v, data: new Date().toISOString(), obs: 'Ajuste Manual' });
...
app.iniciarEscutaLixeira = function() {
    app.db.collection('lixeira_auditoria').where('tenantId', '==', app.t_id).onSnapshot(snap => {
        const tb = document.getElementById('tabelaLixeiraCorpo');        if (tb) tb.innerHTML = snap.docs.map(d => {
            const l = d.data();
...
app.apagarOS = async function() {
    if (app.t_role !== 'admin') { app.showToast("Bloqueado.", "error"); return; }
    const m = prompt("Justificativa para Auditoria:");
    if (!m || m.trim() === '') return;
    const id = document.getElementById('os_id').value;
...
app.exportarPDFMenechelli = async function() {
    ...
    try {
        ...
        doc.autoTable({ startY: y, head: [['Serviço', 'Qtd', 'Unit', 'Subtotal']], body: tableBody, theme: 'grid', headStyles: { fillColor: [30, 41, 59] }, margin: { left: 15, right: 15 } }); y = doc.lastAutoTable.finalY + 15;
        if (app.fotosOSAtual && app.fotosOSAtual.length > 0) {
            if (y > 220) { doc.addPage(); y = 20; }
            doc.setFont("helvetica", "bold"); doc.setFontSize(12); doc.text(`EVIDÊNCIAS FOTOGRÁFICAS`, 15, y); doc.line(15, y + 2, pageWidth - 15, y + 2); y += 10;
        ...
        doc.save(`OS_Oficina_${placa}_${new Date().getTime()}.pdf`); app.showToast("PDF gerado!", "success");
    } catch (erro) { app.showToast("Erro ao gerar PDF.", "error"); } finally { btn.innerHTML = '<i class="bi bi-file-earmark-pdf-fill me-1"></i> Imprimir Laudo'; btn.disabled = false; }
};

// =====================================================================
// 11. INTELIGÊNCIA ARTIFICIAL (GEMINI 2.5 FLASH - LÓGICA DO USUÁRIO)
// =====================================================================
...
app.jarvisAnalisarRevisoes = async function() {
    ...
    const historicoMorto = app.bancoOSCompleto.filter(o => o.status === 'entregue');    if(historicoMorto.length === 0) { div.innerHTML = '<span class="text-white-50">Não há registros suficientes.</span>'; return; }
    ...
    const promptRadar = `Você é o Gestor de Remarketing Automotivo da oficina ${app.t_nome}. Leia o banco de dados abaixo de ordens de serviço finalizadas. Encontre os clientes que fizeram serviço há mais tempo para oferecermos revisões preventivas. Retorne uma lista formatada em HTML (com <ul> e <li>) explicando por que ligar para cada um. BANCO DE DADOS:\n${dadosParaIA}`;
