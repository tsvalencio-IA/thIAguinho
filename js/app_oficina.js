...
app.processarFaturamentoCompleto = async function() {
    ...
    if (app.osParaFaturar.mecanicoAtribuido) {
        ...
        mecanicosArray.forEach(nomeMec => {
            // Tenta encontrar o funcionário ignorando espaços extras
            let func = app.bancoEquipe.find(f => f.nome.trim() === nomeMec.trim());
            
            if (func) {
                // Pega as porcentagens de comissão, tratando strings e valores nulos
                let percMO = parseFloat(func.comissao || func.comissao_mo || 0);
                let percPc = parseFloat(func.comissao_pecas || 0);
                
                // Valores base da O.S.
                let maoObraTotal = parseFloat(app.osParaFaturar.maoObraTotal || 0);
                let pecasTotal = parseFloat(app.osParaFaturar.pecasTotal || 0);
                
                // Cálculo proporcional ao número de mecânicos
                let valMO = (maoObraTotal / qtdeMec) * (percMO / 100);
                let valPc = (pecasTotal / qtdeMec) * (percPc / 100);
                ...
                if(totalMec > 0) {
                    let fDocComissao = {
                        ...
                        tipo: 'SAIDA', 
                        categoria: 'COMISSAO', 
                        desc: `Comissão O.S: [${app.osParaFaturar.placa}] - ${func.nome}`,
                        valor: totalMec, 
                        parcelaAtual: 1, 
                        totalParcelas: 1, 
                        metodo: 'Dinheiro', 
                ...
                console.warn(`[Comissão] Funcionário não encontrado: ${nomeMec}`);
            }
        });
    }
...
app.chamarGemini = async function(promptCompleto) {
    ...
    if (!app.minhaGeminiKey) {
        throw new Error("Chave da API Gemini não configurada para esta oficina.");
    }
    ...
    try {
        ...
        if (!res.ok) {
            const msgErro = data.error ? data.error.message : "Erro na requisição à API do Google";
        ...
        console.error("🔥 [IA] Falha na chamada:", e); 
        throw e; 
    }
};
